...
- **OpenTelemetry instrumentation** — traces and metrics emitted with
  `vertical` attribute pre-set. ⚠️ partial — HTTP, Kafka, LLM-call, and (as of 2026-08-08) tool-call
  spans are all real now. A per-vertical attribute isn't pre-set on any of them today — nothing in
  `LlmClient` or `ToolInvoker` currently knows which vertical is calling — so this would still need
